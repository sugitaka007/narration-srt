...
test("GPT修正用JSONへ選択指示・固有ID・順番・修正前文章・必須条件を保存する", () => {
  ...
  const { exchange } = createRewriteExchange(
    project,
    { ...DEFAULT_REWRITE_PREFERENCES, targetAudience: "画面から廃止された旧設定" },
    new Date("2026-02-03T04:05:06.000Z"),
    "request_test",
  );
  ...
  assert.equal(exchange.rewriteRequest.targetAudience, "");
  assert.doesNotMatch(exchange.rewriteRequest.instructionForGPT, /想定する視聴者/);
